...
class GraficasExport:
    ...
    def crear_figura_principal(R):
        ...
        if R.delta_LP_x is not None and len(R.delta_LP_x) == len(x):
            ...
            ax4.plot(x, np.array(R.delta_LP_x)*1000, color='#EF4444', lw=2.5,
                     label=f'δLP = {R.delta_LP_mm:.2f} mm  (L/{int(sum(R.L_list)*1000/R.delta_LP_mm):.0f})')
            has_data = True
        if has_data:
            ax4.invert_yaxis()
    ...
    def crear_figura_seccion(R):
        ...
        # Bar radius in drawing units = (db_mm/2) / 10.0 / 10.0
        # i.e. db_mm → cm (/10) → drawing units (/10)
        # Clamped to 80% of cover so bars never visually overflow the section
        def bar_r(db_mm):
            return min((db_mm / 2.0) / 100.0, rec * 0.80)
        ...
        r_mi = bar_r(db_mi)
        r_ms = bar_r(db_ms)
        r_gi = bar_r(db_gi) if db_gi > 0 else 0
        r_gs = bar_r(db_gs) if db_gs > 0 else 0

        # Number of bars from real separation (c/sep_cm)
        sep_inf_u = MALLAS[R.malla_inf]["sep"] / 10.0    # cm → drawing units
        sep_sup_u = MALLAS[R.malla_sup]["sep"] / 10.0
        n_bars_inf = max(2, min(int(b / max(sep_inf_u, r_mi*2+0.05)), 16))
        n_bars_sup = max(2, min(int(b / max(sep_sup_u, r_ms*2+0.05)), 16))
        sp_inf = b / (n_bars_inf + 1)
        sp_sup = b / (n_bars_sup + 1)
        rect = mpatches.Rectangle((0, 0), b, h,
