...
def calcular_losa(datos_entrada):
    ...
    R.wl_serv = R.cv
    R.wd_kn = R.wd_serv * 0.00981
    R.wl_kn = R.wl_serv * 0.00981
    ...
    ws_serv_kn = (R.wd_serv + R.cv) * 0.00981
    ...
    A_pos = 2.0 * dc_pos * s_barra_inf
    R.z_pos, _, _, R.cumple_fisura_pos, _ = ME.verificar_fisuracion(R.fs_pos, dc_pos, A_pos)
    if n_vanos > 1 and R.as_sup > 0:
        ...
        A_neg = 2.0 * dc_pos * s_barra_sup
        R.z_neg, _, _, R.cumple_fisura_neg, _ = ME.verificar_fisuracion(R.fs_neg, dc_pos, A_neg)
    else:
        R.cumple_fisura_neg = True

    db_inf = MALLAS[R.malla_inf]["diametro"]
    db_sup = MALLAS[R.malla_sup]["diametro"]
    R.Ld_inf = ME.verificar_longitud_desarrollo(R.fc, R.fy, db_inf, dc_pos, es_superior=False)
    R.Ld_sup = ME.verificar_longitud_desarrollo(R.fc, R.fy, db_sup, dc_pos, es_superior=True)
    L_min = min(R.L_list)
    ...
    ws_total_kn = (R.wd_serv + R.cv) * 0.00981
