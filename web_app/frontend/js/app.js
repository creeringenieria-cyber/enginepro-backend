...
function renderSectionSVG(R) {
    ...
    const nInf = Math.max(2, Math.min(Math.floor(b_draw/Math.max(sep_inf_px,rMI*2+2)), 20));
    const nSup = Math.max(2, Math.min(Math.floor(b_draw/Math.max(sep_sup_px,rMS*2+2)), 20));
    const hd = h_draw_clamped; // alias corto
    ...
    // INF bars — position exactly at separation spacing
    const spInf = b_draw/(nInf+1);
    for(let i=0;i<nInf;i++){
        const bx=x0+(i+1)*spInf, by=y0+hd-rec;
        svg+=`<circle cx="${bx}" cy="${by}" r="${rMI}" fill="${colInf}" stroke="${colInfDk}" stroke-width="1" filter="url(#gi)"/>`;
    }
    if(rGI>0){
        const nGI=Math.max(1,nInf-1);
        const spGI=b_draw/(nGI+1);
        for(let i=0;i<nGI;i++){
            const bx=x0+(i+1)*spGI+spGI/2, by=y0+hd-rec;
            if(bx<x0+b_draw-rGI) svg+=`<circle cx="${bx}" cy="${by}" r="${rGI}" fill="${colInf}" stroke="${colInfDk}" stroke-width="0.8" opacity="0.8"/>`;
        }
    }

    // SUP bars
    const spSup = b_draw/(nSup+1);
    for(let i=0;i<nSup;i++){
        const bx=x0+(i+1)*spSup, by=y0+rec;
        svg+=`<circle cx="${bx}" cy="${by}" r="${rMS}" fill="${colSup}" stroke="${colSupDk}" stroke-width="1" filter="url(#gs)"/>`;
    }
    if(rGS>0){
        const nGS=Math.max(1,nSup-1);
        const spGS=b_draw/(nGS+1);
        for(let i=0;i<nGS;i++){
            const bx=x0+(i+1)*spGS+spGS/2, by=y0+rec;
